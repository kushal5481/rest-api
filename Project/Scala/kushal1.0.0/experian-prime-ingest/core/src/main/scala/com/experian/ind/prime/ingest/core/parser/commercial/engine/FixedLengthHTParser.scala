...
final class FixedLengthHTParser(context: PipelineContext) {
  ...
  def parseFile(
    ...
    logger.info(s"[FixedLengthHTParser] Loading all active SQL rules from path='${rulesPathTrimmed.get}' for ruleIds=[${ruleIdsTrimmed.mkString(",")}]")
    ...
    val rulesList: Seq[RuleDefinition] = if (rulesDS != null) {
      ...
      logger.info(s"[FixedLengthHTParser] Loaded ${rules.size} active rules from '${rulesPathTrimmed.get}'")
      rules
    } else {
