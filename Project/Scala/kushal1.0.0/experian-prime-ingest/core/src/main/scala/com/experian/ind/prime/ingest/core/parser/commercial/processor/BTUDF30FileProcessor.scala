...
/**
 * BTUDF30 File Processor for Commercial Bureau v3.0 files
 */
object BTUDF30FileProcessor {
  
  private lazy val logger = DualLogger(getClass)

  /**
   * Process a BTUDF30 file through splitting and parsing stages
   * 
   * Workflow:
  ...
  def processFile(context: PipelineContext): Try[ProcessingResult] = {
    ...
    logger.info(s"[PROCESSOR] Starting BTUDF30 file processing. fileLoadId=$fileLoadId, fileName=$inputFileName, format=$fileFormat")
    ...
    Try {
      ...
      stopwatch.stop()
      logger.info(s"[PROCESSOR] BTUDF30 file processing completed successfully. fileLoadId=$fileLoadId, elapsedTime=${stopwatch.elapsed()}, totalDataFrames=${batchDataFrames.size}, df1RowCount=$df1RowCount")
      result
    } recover {
      case ex: Exception =>
        stopwatch.stop()
