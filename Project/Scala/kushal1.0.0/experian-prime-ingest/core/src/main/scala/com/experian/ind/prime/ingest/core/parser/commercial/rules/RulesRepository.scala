...
final class RulesRepository(spark: SparkSession) {
  ...
  /**
   * GENERIC FRAMEWORK: Load ALL active rules for a given set of rule IDs.
   *
   * This is the preferred method for loading rules.
   *
   * Returns:
   *   - All rules where rule_engine_type = "sql"
   *   - Filtered to ACTIVE status only
   *   - Sorted by rule_priority (ascending), then by rule_id (for deterministic ordering)
   *
   * BENEFITS:
   *   - Single I/O pass (loads rules file once)
   ...
   * @return Dataset of RuleDefinition objects (SQL-only, pre-sorted by priority)
   */
  def loadActiveRules(path: String, allowedIds: Seq[String]): Dataset[RuleDefinition] = {
    ...
    // Filter for ACTIVE status AND SQL engine type
